/**
 * Project Nine 서버 저장 백엔드.
 *
 * nine.elcherlab.com/api/* 를 Caddy 가 여기(127.0.0.1:3700)로 프록시한다.
 ...
 *
 * 저장은 nine.saves 한 줄(user_id 당 jsonb 하나)이 전부다. 게임 규칙은 전부
 * 클라이언트에 있고 서버는 검증하지 않는다 — 1인용 방치형이라 치트 방지보다
 * 단순함을 택했다(pixel-pet 과 같은 판단).
 *
 * 탈퇴 시 삭제 경로가 없는 것도 pixel-pet 과 같다. nine.saves 가 identity.users 를
...
const SCHEMA_SQL = `
  create schema if not exists nine;
  create table if not exists nine.saves (
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'POST' && path === '/internal/export-user') {
      ...
      const r = await pool.query('select data, updated_at from nine.saves where user_id = $1', [userId]);
      ...
      if (en) {
        return json(res, 200, {
          service: 'Project Nine (nine.elcherlab.com)',
      ...
      return json(res, 200, {
        서비스: 'Project Nine (nine.elcherlab.com)',
    ...
    if (req.method === 'GET' && path === '/api/save') {
      const r = await pool.query('select data from nine.saves where user_id = $1', [s.uid]);
    ...
    if (req.method === 'PUT' && path === '/api/save') {
      ...
      await pool.query(
        `insert into nine.saves (user_id, data, updated_at) values ($1, $2, now())
    ...
    if (req.method === 'DELETE' && path === '/api/save') {
      await pool.query('delete from nine.saves where user_id = $1', [s.uid]);
      return json(res, 200, { ok: true });
    }
...
try {
  ...
  console.log('nine.saves 스키마 확인 완료');
} catch (e) {
...
server.listen(PORT, HOST, () => {
  console.log(`⚡ Project Nine 서버 저장 실행 중: http://${HOST}:${PORT} (쿠키 ${COOKIE_NAME})`);
});
